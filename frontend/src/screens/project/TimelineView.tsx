...
export function TimelineView({
  ...
  const rows = useMemo(() => {
    const out: { wpTitle: string; task: TaskWithBlocked; date: string | null }[] = [];
    (wps.data ?? []).forEach((wp, i) => {
      const tasks = taskQueries[i]?.data ?? [];
      for (const t of tasks) out.push({ wpTitle: wp.title, task: t, date: scheduledDate.get(t.id) ?? null });
    });
    return out;
  }, [wps.data, taskQueries, scheduledDate]);
  ...
  const gridCols = `220px repeat(${axis.days.length}, ${COL_W}px)`;
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <p className="text-[11px] font-semibold text-text-tertiary">
        Drag a flexible bar to a new day to <strong>re-plan from there</strong> (it proposes a change —
        it never silently moves the task). Pinned <span className="text-text-secondary">◆</span> bars
        can’t be dragged.
      </p>
            ...
            {axis.days.map((d) => {
              ...
              return (
                <div
                  ...
                  onDragOver={(e) => {
                    if (date && !task.is_time_fixed) e.preventDefault();
                  }}
                  onDrop={() => date && onDropOnDay(task, date, d)}
                >
                  {here && (
                    <div
                      draggable={!task.is_time_fixed}
                      onDragStart={(e) => e.dataTransfer.setData("text/plain", task.id)}
                      title={
                        task.is_time_fixed
                          ? `Pinned to ${task.fixed_date ?? date} — time-fixed, can’t be dragged. Edit it in the work-package sheet.`
                      ...
                      className={cn(
                        ...
                        task.status === "done" && "bg-progress-soft text-progress",
                      )}
                    >
                      {task.is_time_fixed ? <span aria-hidden>◆</span> : <StatusPill status={taskStatusKind(task)} />}
                      <span className="truncate">{formatEstimate(task.estimate_hours, task.difficulty)}</span>
                    </div>
                  )}
                </div>
              );
