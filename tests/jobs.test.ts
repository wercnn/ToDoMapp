 ...
 *  - cross-trigger backoff: slippage does NOT clobber a pending user_request proposal;
...
describe("slippage detector", () => {
  ...
  it("backs off rather than clobbering a pending user_request proposal", async () => {
    const { ctx } = await provision({ timezone: "UTC" });
    const now = new Date("2026-06-14T12:00:00Z");
    const sc = await seedScenario(db, ctx, { planDate: "2026-06-10" });
    await confirmDay(sc.dayId, now);

    // The user already has a pending proposal they may be about to approve.
    ...
    expect(res.slippedDayIds).toEqual([sc.dayId]); // day still slips
    expect(res.proposalCreated).toBe(false); // but no slippage proposal piled on

    const pending = await pendingProposals(ctx.workspaceId);
    expect(pending).toHaveLength(1);
    expect(pending[0]!.id).toBe(userProposal.id);
    expect(pending[0]!.trigger).toBe("user_request");
  });
});
