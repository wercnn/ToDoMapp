...
| `GET` | `/projects/{projectId}/flow` | **Project Flow Diagram** payload (§4.2, Principle 5). | – | `200` `{ nodes: [wp/task + derived_status(done\|in_progress\|blocked\|open)], edges: [position_task_edges, wp_deps], critical_path: [task_id…], next_milestone }` | Fully derived: blocked = incomplete ordered predecessors; task edges come from position inside each work package, WP edges come from `work_package_dependency`, and critical path is computed from that graph + estimates to next unachieved milestone (data-model §6). Nothing is written. | read `work_package`, `task`, `work_package_dependency`, `milestone` |
...
The planning unit for estimation and dependencies; a to-do list object (§3.1). **Tables:** `work_package`.
...
| `DELETE` | `/work-packages/{wpId}` | Delete a work package and its tasks. | – | `204` | Cascades tasks, dependency edges, and their plan items. Replanning after deletion is manual. | `work_package` → cascade `task`, dep tables, `daily_plan_item` |
...
| Method | Route | Description | Input | Output | Behavior | Tables |
|---|---|---|---|---|---|---|
| `GET` | `/work-packages/{wpId}/tasks` | List a work package's to-do lines. | Query: `status?` | `200` `[task + blocked]` | `blocked` is derived from earlier incomplete tasks in the same work package plus incomplete upstream work packages — never stored. | `task`, `work_package_dependency` |
| `POST` | `/work-packages/{wpId}/tasks` | Create a task (Workspace full edit; Companion "quick add", §5 surface table). | `{ title, notes?, estimate_hours? XOR difficulty?, is_time_fixed?, fixed_date?, position? }` | `201` `task` | Same either/or estimation + time-fixed pairing CHECKs as work packages. New tasks are placed on a concrete holding day, but no replan proposal is created automatically. | `task`, `daily_plan_day`, `daily_plan_item` |
| `GET` | `/tasks/{taskId}` | Read one task. | – | `200` `task + blocked` | Derived blocked flag included. | `task`, `work_package_dependency` |
...
Task order is position-based inside each work package. Work-package edges remain explicit "must finish before" scheduling constraints. Task dependency rows are retained for compatibility/legacy metadata only; task scheduling, blocked-state, flow edges, and critical path all use position order. Acyclicity for stored edge rows is an **API-layer** reachability check on every insert (no DB triggers, §9.2 rule 6). **Tables:** `task_dependency`, `work_package_dependency`.

| Method | Route | Description | Input | Output | Behavior | Tables |
|---|---|---|---|---|---|---|
| `POST` | `/task-dependencies` | Create legacy task-edge metadata. | `{ predecessor_task_id, successor_task_id }` | `201` `edge` | Rejects self-dependency (`422`), duplicate edge (`409`, PK), and **any cycle** (`409` after graph reachability check — invariant #1). This edge does not affect blocked-state, replanning order, flow edges, or critical path. | `task_dependency` |
| `DELETE` | `/task-dependencies/{predecessorTaskId}/{successorTaskId}` | Remove a legacy task edge. | – | `204` | Removes compatibility metadata. Task blocked-state remains position-derived. | `task_dependency` |
...
| Method | Route | Description | Input | Output | Behavior | Tables |
|---|---|---|---|---|---|---|
| `GET` | `/roadmap` | The Duolingo-style path: persisted past/confirmed days + computed projection, milestones as landmarks, "you are here" (§4.3). | Query: `from?`, `to?`, `goal_id?` | `200` `{ days: [persisted ∪ projected], milestones: [{ id, title, achieved, achieved_date, projected_date }], position: { today, current_streak } }` | Persisted days come from the tables; an achieved milestone has no incomplete tasks left to project, so it carries `achieved_date` (from `achieved_at`) and the landmark is dated by `achieved_date ?? projected_date`; days beyond the confirmed horizon are **re-projected live** from WBS + dependencies + estimates + capacity + time-fixed dates (data-model §6) and flagged `projected: true`. Nothing is written. | `daily_plan_day`, `daily_plan_item`; read `task`, `work_package`, deps, `project`, `milestone`, `user_stats` |
...
| `GET` | `/morning-brief` **⚡eng** | **The signature composite read** behind the morning notification (§4.6, Journey B/D). | – | `200` `{ today: { day, items } \| null, stats, pending_proposal: { id, summary } \| null, pending_replan: ReplanProposalDetail \| null, recovery: ReplanRecoveryMeta \| null, position, next_milestone }` | One call gives the Companion everything for the wake-up moment: today's Daily Goals, the full stats row (**points AND streak** — §4.6 surfaces both), backward-compatible `pending_proposal`, full `pending_replan` detail, same-day recovery metadata, roadmap position, and the nearest milestone (by earliest projected_date). `today` is `null` when no day is persisted for today — the wake-up read never 404s. Opening it records engagement. | `daily_plan_day`, `daily_plan_item`, `task`, `user_stats`, `replan_proposal` (partial pending index), `milestone`, `engagement_day` |
...
| `POST` | `/replan-proposals` | User-initiated replan (deep replans on web, §5). | `{ trigger: 'user_request', scope?: { project_id?, from_date? } }` | `201` `proposal` (pending) | Engine analyzes dependency impact, milestone/deadline risk, classifies flexible vs. time-fixed, produces `summary` + `changes`. Any older pending proposal is marked `expired` (superseded, data-model §9.4). Normal replans are manual; slippage recovery proposals are created by the system job for morning review. | `replan_proposal`; reads graph + plan tables |
...
| `POST` | `/replan-proposals/{proposalId}/recovery-preview` | Recompute a slippage recovery proposal with morning choices. | `{ today_task_ids: string[], time_fixed_resolutions?: TimeFixedResolution[] }` | `200` `ReplanProposalDetail` | Read-only preview. Selected flexible slipped tasks are pinned to local today; unselected slipped tasks get an earliest date of tomorrow. Dependency-invalid selections surface as blocking conflicts. | `replan_proposal`; reads graph + plan tables |
| `POST` | `/replan-proposals/{proposalId}/recovery-apply` **⚡eng** | Apply the morning recovery choices. | `{ today_task_ids: string[], time_fixed_resolutions?: TimeFixedResolution[] }` | `200` `ReplanProposalDetail` | Re-runs analysis with the submitted choices, rejects unresolved time-fixed or dependency-invalid today selections, applies the generated diff, stores recovery metadata in `applied_changes`, and records engagement. | `replan_proposal`, `daily_plan_day`, `daily_plan_item`, `engagement_day`, `user_stats` |
...
| `task_dependency`, `work_package_dependency` | §9 | Task dependency rows are retained for compatibility metadata; scheduling, blocked checks, flow edges, and critical path use task position inside a work package plus `work_package_dependency`. |
| `daily_plan_day`, `daily_plan_item` | §10; §11 approve-apply; §8 completion mirror; §13 slippage marking | §10, morning brief, §11 analysis |
| `replan_proposal` | §11; §13 slippage job | §11, morning brief |
