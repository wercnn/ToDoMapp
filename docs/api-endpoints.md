...
| `GET` | `/morning-brief` **⚡eng** | **The signature composite read** behind the morning notification (§4.6, Journey B/D). | – | `200` `{ today: { day, items } \| null, stats: { total_points, current_streak, longest_streak, last_engaged_date }, pending_proposal: { id, summary } \| null, position: { today, current_streak }, next_milestone: { id, title, projected_date, days_away } \| null }` | One call gives the Companion everything for the wake-up moment: today's Daily Goals, the full stats row (**points AND streak** — §4.6 surfaces both), any pending recovery proposal headline (`summary`, e.g. *"2 tasks from yesterday are pending…"*), roadmap position, and the nearest milestone (by earliest projected_date). `today` is `null` when no day is persisted for today — the wake-up read never 404s. Opening it records engagement. | `daily_plan_day`, `daily_plan_item`, `task`, `user_stats`, `replan_proposal` (partial pending index), `milestone`, `engagement_day` |
